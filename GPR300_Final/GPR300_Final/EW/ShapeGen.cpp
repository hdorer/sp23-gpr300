...
namespace ew {
	...
	void createCube(float width, float height, float depth, MeshData& meshData)
	{
		...
		Vertex vertices[24] = {
			...
			//Bottom face
			{glm::vec3(-halfWidth, -halfHeight, -halfDepth), glm::vec3(0,-1,0), glm::vec2(0, 0), glm::vec3(1, 0, 0)}, //BL
			{glm::vec3(+halfWidth, -halfHeight, -halfDepth), glm::vec3(0,-1,0), glm::vec2(0, 1), glm::vec3(1, 0, 0)}, //BR
			{glm::vec3(+halfWidth, -halfHeight, +halfDepth), glm::vec3(0,-1,0), glm::vec2(1, 1), glm::vec3(1, 0, 0)}, //TR
			{glm::vec3(-halfWidth, -halfHeight, +halfDepth), glm::vec3(0,-1,0), glm::vec2(0, 1), glm::vec3(1, 0, 0)}, //TL
		};
	...
	void createSphere(float radius, int numSegments, MeshData& meshData)
	{
		...
		meshData.vertices.push_back({ glm::vec3(0,topY,0), glm::vec3(0,1,0), glm::vec2(0.5, 1), glm::vec3(1, 0, 0) });
		...
		meshData.vertices.push_back({ glm::vec3(0,bottomY,0), glm::vec3(0,-1,0), glm::vec2(0.5, 0), glm::vec3(1, 0, 0) });
	...
	void createCylinder(float height, float radius, int numSegments, MeshData& meshData)
	{
		...
		for (int i = 0; i <= numSegments; i++)
		{
			...
			glm::vec2 uv = glm::vec2(cos(i * thetaStep) * 0.5 + 0.5, sin(i * thetaStep) * 0.5 + 0.5);
		...
		for (int i = 0; i <= numSegments; i++)
		{
			...
			glm::vec2 uv = glm::vec2(cos(i * thetaStep) * 0.5 + 0.5, 1.0 - (sin(i * thetaStep) * 0.5 + 0.5));
		...
		for (int i = 0; i <= numSegments; i++)
		{
			...
			glm::vec2 uv = glm::vec2((float)i / numSegments, 1);
			glm::vec3 tangent = glm::normalize(glm::cross(normal, glm::vec3(0, 1, 0)));

			meshData.vertices.push_back(Vertex(pos, normal, uv, glm::vec3(1, 0, 0)));
		}
		//Side bottom ring
		for (int i = 0; i <= numSegments; i++)
		{
			...
			glm::vec2 uv = glm::vec2((float)i / numSegments, 0);
			meshData.vertices.push_back(Vertex(pos, normal, uv, glm::vec3(1, 0, 0)));
		}
